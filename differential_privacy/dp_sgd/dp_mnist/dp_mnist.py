...
# training.
tf.flags.DEFINE_string("accountant_type", "Moments","Moments, Amortized, Baseline")
...
def Train(mnist_train_file, mnist_test_file, network_parameters, num_steps,
  ...
  with tf.Graph().as_default(), tf.Session() as sess, tf.device('/cpu:0'):
    ...
    elif FLAGS.accountant_type == "Baseline":
      with_privacy=0
    ...
    if network_parameters.projection_type == "PCA":
      with tf.variable_scope("pca"):
        ...
        if with_privacy:
            ...
            pca_projection = dp_pca.ComputePrincipalProjection(
                all_data, network_parameters.projection_dimensions)
    ...
    for step in xrange(num_steps):
      ...
      if (eval_steps > 0 and (step + 1) % eval_steps == 0) or should_terminate:
        ...
        tracurr_time=time.time()
        runtime=tracurr_time-train_start
        print('The training rumtime is=%.3f s' % runtime)
        ...
        #time_end = time.clock()
        testingtime = curr_time - tracurr_time
        print('The testing rumtime is=%.3f s' % testingtime)
